...
int main(int argc, char *argv[])
{
   ...
   ifstream in((string("HiggsFiles/Signal/HiggsFile_") + argv[1]).c_str());
   while(in)
   {
   ...
   string suffixes[] = {"Efficiency", "PhaseSpace", "Scalar", "Pseudoscalar"};
   vector<string> Suffixes(suffixes, suffixes + SuffixCount);

   string OutputLocation = "/wntmp/yichen";   // Change this to something else!
            ...
            for(int iMass1 = 0; iMass1 < NBinsMass1; iMass1++)
            {
               for(int iMass2 = 0; iMass2 < NBinsMass2; iMass2++)
               {
                  for(int iPhiOffset = 0; iPhiOffset < NTriesPhiOffset; iPhiOffset++)
                  {
                     ...
                     // Prepare input parameters - no need for symmetrization since Phi0 and Theta0 are obsolete
                     ...
                     // Calculate lepton vectors
                     LeptonVectors Leptons1 = ConvertAnglesToVectors(event, HiggsPT, HiggsEta, true);
                     ...
                     double PhaseSpace = GetPhaseSpace(HMass, Z1, Z2);
                     double MEWeight1 = GetM2Scalar(HMass, Z1, Z2, 0, 0, Phi, cos(Theta1), cos(Theta2));
                     double MEWeight2 = GetM2PseudoScalar(HMass, Z1, Z2, 0, 0, Phi, cos(Theta1), cos(Theta2));

                     MEWeight1 = MEWeight1 * Z1 * Z2 * 4;   // The notorious Jacobian factor
                     MEWeight2 = MEWeight2 * Z1 * Z2 * 4;

                     MEWeight1 = MEWeight1 * PhaseSpace;
                     MEWeight2 = MEWeight2 * PhaseSpace;

                     // Put things in the correct mass bins
                     PassedTries[0][iZ1MassNew][iZ2MassNew] += LeptonEfficiency * Weight;
                     PassedTries[1][iZ1MassNew][iZ2MassNew] += LeptonEfficiency * Weight * PhaseSpace;
                     PassedTries[2][iZ1MassNew][iZ2MassNew] += LeptonEfficiency * Weight * MEWeight1;
                     PassedTries[3][iZ1MassNew][iZ2MassNew] += LeptonEfficiency * Weight * MEWeight2;
